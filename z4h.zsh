# 如果TERM环境变量未设置，则设置为xterm-256色
'[' '-n' "${TERM-}" ']' || 'export' TERM='xterm-256color'

# 如果在Windows Terminal中运行，设置COLORTERM为truecolor以启用真彩色支持
'[' '-n' "${WT_SESSION-}" ']' && 'export' COLORTERM="${COLORTERM:-truecolor}"

# 检查是否在zsh中运行
if '[' '-n' "${ZSH_VERSION-}" ']'; then
  # 防止用户使用source命令加载zshenv
  # 这会导致配置不完整，应该使用exec zsh重新启动shell
  ...
  # 标记z4h.zsh已被source过，防止重复加载
  'typeset' '-gri' _z4h_source_called='1'

  # 设置zsh模拟模式，确保脚本在一致的环境中运行
  'emulate' 'zsh'

  # 设置各种zsh选项，优化用户体验
  'setopt'                                                                     \
    'always_to_end'          'auto_cd'                'auto_param_slash'       \  # 光标移动和目录操作
    'auto_pushd'             'c_bases'                'auto_menu'              \  # 目录栈和补全菜单
    'extended_glob'          'extended_history'       'hist_expire_dups_first' \  # 扩展通配符和历史记录处理
    'hist_find_no_dups'      'hist_ignore_dups'       'hist_ignore_space'      \  # 历史记录去重和过滤
    'hist_verify'            'interactive_comments'   'multios'                \  # 历史验证和多重重定向
    'no_aliases'             'no_bg_nice'             'no_bg_nice'             \  # 禁用别名和后台进程优先级调整
    'no_flow_control'        'no_prompt_bang'         'no_prompt_subst'        \  # 禁用流控制和提示符扩展
    'prompt_cr'              'prompt_percent'         'prompt_sp'              \  # 提示符格式设置
    'share_history'          'typeset_silent'         'hist_save_no_dups'      \  # 历史记录共享和变量声明
    'no_auto_remove_slash'   'no_list_types'          'no_beep'                   # 禁用自动删除斜杠和蜂鸣声

  # 设置临时文件目录
  # 如果TMPPREFIX的父目录不存在，尝试设置一个可用的临时目录
...
%F{%(?.2.1)}%#%f%b "
    ...
    # 设置单词字符和历史记录参数
    WORDCHARS=''                # 定义哪些字符被视为单词的一部分
    ZLE_REMOVE_SUFFIX_CHARS=''  # 定义自动删除的后缀字符
    HISTSIZE='1000000000'       # 历史记录大小（内存中）
    SAVEHIST='1000000000'       # 保存到文件的历史记录大小

    # 设置历史文件路径
    if '[' '-n' "$HISTFILE" ']'; then
      # 如果用户已经设置了HISTFILE，记录这一点
